...
function Sync(method, model, opts) {
    ...
    switch(method) {
        case 'create' :
            ...
            apiCall(params, function(_response) {
            	if (!_response.success) {
            		_response = fixResponseJSON(_response);
            	}
        ...
        case 'update' :
			...
			//always add the node type when it's sent
			var nodeType = model.get('type') ? model.get('type') : ""; //TODO add default node type???
			if (nodeType.length > 0) {
				var data = {"type" : nodeType };				
			} else{
				var data = {};				
			}
...
function fixResponseJSON(response) {
	var responseText = response.responseText;
	if (responseText != null && responseText.length > 0) {
		var endIndex = responseText.lastIndexOf("}");
		if (endIndex != -1) {
			responseText = responseText.substr(0, endIndex + 1);
		}
		response.responseText = responseText;
		try {
			response.responseJSON = JSON.parse(responseText);
			response.success = true;
			Ti.API.info("fixed JSON response. Moving on...");
		} catch(e) {
			Ti.API.info("failed to fix JSON response: " + JSON.stringify(e));
		}		
	}
	return response;
}
